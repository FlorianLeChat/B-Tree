...
public class Noeud<Type> implements java.io.Serializable
{
	...
	public HashMap<Integer, Integer> pointeurs = new HashMap<>();
	...
	private Executable<Integer> compar;
	 ...
	 */
	public Noeud(int u, Executable<Integer> e, Noeud<Type> parent)
	{
		this.u = u;
		this.tailleMin = u / 2;
		compar = e;
	...
	public boolean compare(Integer arg1, Integer arg2)
	{
	 ...
	 */
	public Noeud<Type> contient(Integer valeur)
	{
	...
	public Noeud<Type> contientIndex(String valeur)
	{
		Noeud<Type> retour = null;

		for (Integer key : keys)
		{
			String val = key.toString().replace("[", "").split(",")[0];

			if (val.equals((String) valeur))
			{
				return this;
			}
			else
			{
				...
				while ((trouve == null) && (i < this.fils.size()))
				{
					trouve = this.fils.get(i).contientIndex(valeur);
					i++;
				}
	 ...
	 * @param nouvelleValeur que l'on souhaite ins�rer
	 * @return le Noeud choisi
	 */
	public Noeud<Type> choixNoeudAjout(String nouvelleValeur)
	{
		Noeud<Type> retour = null;
		...
		else
		{
			...
			{
				trouve = compare(Integer.parseInt(nouvelleValeur.split(",")[1]), this.keys.get(index));

				if (!trouve)
					index++;
			}

			retour = this.fils.get(index).choixNoeudAjout(nouvelleValeur);
		}
	 ...
	 * @param nouvelleValeur � ajouter aux clefs du noeud courant
	 */
	private void insert(String nouvelleValeur)
	{
		int i = 0;
		int key = Integer.parseInt(nouvelleValeur.split(",")[1]);
		int nligne = Integer.parseInt(nouvelleValeur.split(",")[0]);

		while ((this.keys.size() > i) && compare(this.keys.get(i), key))
		{
			i++;
		}

		this.keys.add(i, key);
		this.pointeurs.put(key, nligne);
	}
	 ...
	 */
	private void removeKey(Integer valeur)
	{
		this.keys.remove(valeur);
		this.pointeurs.remove(valeur);
	}
	...
	public Noeud<Type> addValeur(String nouvelleValeur)
	{
	 ...
	 */
	public Noeud<Type> removeValeur(Integer valeur, boolean force)
	{
		...
		if (keyCount < tailleMin)
		{
			...
			if (noeud.parent != null)
			{
				...
				Integer remplacant = null;
				Integer nbligneremp = null;
				Integer valeurARemplacer = null;
				...
				if (suivant != null && suivant.keys.size() > tailleMin)
				{
					...
					nbligneremp = suivant.pointeurs.get(remplacant);
					// On ajoute la premi�re clef du noeud suivant au noeud courant
					noeud.keys.add(remplacant);
					noeud.pointeurs.put(remplacant, nbligneremp);
					// Et on la retire des clefs du noeud suivant
					suivant.keys.remove(remplacant);
					suivant.pointeurs.remove(remplacant);
					// On remplace alors dans les noeuds parents la valeur qui a �t� r�cup�r�e dans
					// le noeud suivant par la nouvelle "premi�re valeur"
					remplacerDansParents(noeud.parent, remplacant, suivant.keys.get(0), suivant.pointeurs.get(suivant.keys.get(0)));
					// Et on remplace la valeur qu'on a effac� par la valeur qui a pris sa place
					remplacerDansParents(noeud.parent, valeur, noeud.keys.get(0), noeud.pointeurs.get(noeud.keys.get(0)));
				}
				else
				{
					...
					if (precedent != null && precedent.keys.size() > tailleMin)
					{
						...
						noeud.addValeur(precedent.pointeurs.get(remplacant) + "," + remplacant, true);
					...
					else // Sinon, on va devoir merge le noeud courant avec le suivant ou le pr�c�dent
					{
						...
						{
							// On mets toutes les clefs restantes dans le noeud courant dans le noeud
							// pr�c�dent tant que ce dernier peut en accueillir
							while (!noeud.keys.isEmpty() && precedent.keys.size() < u)
							{
								Integer valeurADeplacer = noeud.keys.get(0);
								precedent.keys.add(valeurADeplacer);
								precedent.pointeurs.put(valeurADeplacer, noeud.pointeurs.get(noeud.keys.get(0)));
								noeud.pointeurs.remove(noeud.keys.get(0));
								noeud.keys.remove(0);
							}
						}
						else if (!noeud.keys.isEmpty() && suivant != null && suivant.keys.size() < u) // M�me op�ration avec le noeud suivant
						{
							while (!noeud.keys.isEmpty() && suivant.keys.size() < u)
							{
								...
								remplacerDansParents(noeud,suivant.keys.get(1), suivant.keys.get(0), valeurARemplacer);
						...
						else // si pas de pr�c�dent ou de suivant / pas de place / le noeud courant est le seul fils > On r�duit la hauteur
						{
							...
							HashMap<Integer, Integer> pointz = new HashMap<>();

							racine.reequilibrer(keyz, pointz);
							racine.fils.clear();
							racine.keys.clear();
							...
							for (Integer key : keyz)
							{
								...
								{
									Noeud<Type> newRacine = racine.addValeur(pointz.get(key) + "," + key);
		...
		{
			// Si la clef que l'on a effac� �tait pr�sente dans les clefs des noeuds parents, on remplace de mani�re r�cursive
			remplacerDansParents(noeud, valeur, noeud.keys.get(0), noeud.pointeurs.get(noeud.keys.get(0)));
		}
	...
	public void reequilibrer(ArrayList<Integer> keyz, HashMap<Integer, Integer> pointz)
	{
		for (Noeud<Type> noeud : this.fils)
		{
			if (noeud.fils.isEmpty())
			{
				...
				pointz.putAll(noeud.pointeurs);
			}
			else
			{
				for (Noeud<Type> fils : noeud.fils)
					fils.reequilibrer(keyz, pointz);
			}
		}
	}

	public void remplacerDansParents(Noeud<Type> noeud, Integer aRemplacer, Integer remplacant, Integer nligneremplacant)
	{
		if (noeud.keys.contains(aRemplacer))
		{
			noeud.keys.set(noeud.keys.indexOf(aRemplacer), remplacant);
			pointeurs.put(remplacant, nligneremplacant);
		}

		if (noeud.parent != null)
			remplacerDansParents(noeud.parent, aRemplacer, remplacant, nligneremplacant);
	}
	...
	public boolean parentContient(Noeud<Type> noeud, Integer valeur)
	{
		boolean trouve = false;
	 ...
	 * @return la <Noeud>racine</Noeud> de l'arbre
	 */
	public Noeud<Type> addValeur(String nouvelleValeur, boolean force) {
		...
		if (!noeud.keys.contains(Integer.parseInt(nouvelleValeur.split(",")[1])))
		{
			...
			if (tailleListe >= u)
			{
				...
				eleMedianPoint = (int) noeud.pointeurs.get(noeud.keys.get(indexMedian));

				// On utilise un appel r�cursif pour ajouter au noeud gauche, les clefs du noeud courant
				for (int i = 0; i < indexMedian; i++)
					noeudGauche.addValeur(noeud.pointeurs.get(noeud.keys.get(i)) + "," + noeud.keys.get(i));
				...
				if (!noeud.fils.isEmpty())
				{
					for (int i = indexMedian + 1; i < tailleListe; i++)
						noeudDroit.addValeur(noeud.pointeurs.get(noeud.keys.get(i)) + "," + noeud.keys.get(i));
				}
				else
				{
				...
				if (noeud.parent == null)
				{
					...
					nouveauParent.addValeur(eleMedianPoint + "," + eleMedian, true);
				...
				else
				{
					...
					racine = noeud.parent.addValeur(eleMedianPoint + "," + eleMedian, true);
				}
